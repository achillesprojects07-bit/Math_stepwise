...
function makeLessons(level){ return Array.from({length:200},(_,i)=>{ const lessonNumber=i+1; const b=blockFor(level, lessonNumber); return { level, lessonNumber, displayId:`${level}-${lessonNumber}`, title:b.title, blockTitle:b.blockTitle, max:b.max, skill:b.skill, mode:b.mode, blockFrom:b.from, blockTo:b.to }; }); }
...
function recommendationFrom(record){ if(record.practiceItemCount||record.wrongFirstTry||record.slowCorrect){ return { type:'lesson_range', level:record.level, from:record.lessonNumber, to:record.lessonNumber, label:`${record.lesson}: ${record.lessonTitle}`, lessons:[record.lesson], skills:[record.lessonTitle], reason:[record.wrongFirstTry?`${record.wrongFirstTry} corrected item${record.wrongFirstTry===1?'':'s'}`:'', record.slowCorrect?`${record.slowCorrect} slow item${record.slowCorrect===1?'':'s'}`:''].filter(Boolean).join(' • ')||'Light review' }; } return null; }
